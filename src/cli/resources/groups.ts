...
function presentConfig(row: ContainerConfigRow): Record<string, unknown> {
  return {
    ...
    trace_user_id: row.trace_user_id,
  ...
  customOperations: {
    ...
    'config update': {
      ...
      description:
        ...
        'Use --id <group-id> and any of: --provider, --model, --effort, --image-tag, --assistant-name, --max-messages-per-prompt, --cli-scope, --trace-user-id.',
      handler: async (args) => {
        const id = args.id as string;
        if (!id) throw new Error('--id is required');
        const row = getContainerConfig(id);
        ...
        if (args['trace-user-id'] !== undefined || args.trace_user_id !== undefined) {
          updates.trace_user_id = (args['trace-user-id'] ?? args.trace_user_id) as string;
        }

        if (Object.keys(updates).length === 0) {
          throw new Error(
            'Nothing to update — provide at least one of: --provider, --model, --effort, --image-tag, --assistant-name, --max-messages-per-prompt, --cli-scope, --trace-user-id',
          );
        }
